 ...
 * for {@link org.openstreetmap.osm.data.hsqldb.HsqldbCurrentNodeReader} and others.
 * @param <T> te type we are a {@link ReleasableIterator} for.
 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
 */
public abstract class ConstrainedBaseReader<T> implements ReleasableIterator<T> {

    /**
     * my logger for debug and error-output.
     */
    static final Logger LOG = Logger.getLogger(ConstrainedBaseReader.class.getName());
     ...
     */
    public ConstrainedBaseReader(final DatabaseLoginCredentials aLoginCredentials) {
        firstContext = new DatabaseContext(aLoginCredentials);
    }
    ...
    protected ResultSet getResultSet(final String sql) {
        ...
        if (myResultSet == null)
        try {
            myResultSet =  firstContext.executeQuery(sql);  // executeStreamingQuery(sql); // TODO: Check streaming query!
